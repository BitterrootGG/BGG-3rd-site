...
const CertificationCard = ({ title, icon, type, date, points, credential }) => (
  <div className="certification-card bg-tertiary p-6 rounded-2xl w-full h-full flex flex-col justify-between no-select">
    <div>
      ...
      <h3 className="text-stone-light font-bold text-[20px] mb-2 no-select">{title}</h3>
      <p className="text-stone-mid text-[12px] mb-1 no-select">{type}</p>
      <p className="text-stone-mid text-[12px] mb-3 no-select">{date}</p>
      <ul className="list-disc ml-5 space-y-1">
        {points.slice(0, 2).map((point, index) => (
          <li
            key={`certification-point-${index}`}
            className="text-stone-light text-[12px] pl-1 tracking-wider no-select"
          >
            {point}
          </li>
        ))}
    ...
    <div className="mt-4 flex justify-end no-select">
      <a
        ...
        className="border border-forest-moss text-forest-sage hover:bg-forest-mid/70 py-2 px-4 rounded-lg outline-none w-fit text-[12px] font-bold transition-all hover:scale-105 no-select"
      >
        View Credential
      </a>
    </div>
  ...
  return (
    <div ref={sectionRef}>
      ...
      <style>{`
        ...
        .certification-card {
          background-color: rgba(15, 29, 20, 0.92);
        ...
        .black-gradient {
          background: #0b120e;
          background: linear-gradient(120deg, rgba(11, 18, 14, 0.95), rgba(30, 58, 42, 0.85));
        }
        @media (max-width: 768px) {
          ...
          .certification-card {
            background-color: rgba(15, 29, 20, 0.9);
