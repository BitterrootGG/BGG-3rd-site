// Bitterroot Ground & Gravel - Business Constants
// No asset imports needed for business content

export const navLinks = [
  {
    id: "services",
    title: "Services",
  },
  {
    id: "process",
    title: "Process",
  },
  {
    id: "servicearea",
    title: "Service Area",
  },
  {
    id: "contact",
    title: "Contact",
  },
];

const services = [
  {
    title: "Forestry Mulching",
    description: "Mechanical vegetation management for property development, fire mitigation, and land restoration. Designed to efficiently reduce brush and timber while minimizing soil disturbance and erosion.",
  },
  {
    title: "Defensible Space / Fire Fuels Reduction",
    description: "Selective clearing around structures and access routes to reduce wildfire risk. Combines mechanical mulching and hand work where terrain or slope requires alternate methods.",
  },
  {
    title: "Access Creation & Site Preparation",
    description: "New access on vacant land including clearing, subgrade preparation, drainage shaping, and gravel base installation. Built for durability, equipment access, and long-term maintenance.",
  },
  {
    title: "Driveway Installation & Repair",
    description: "New driveway construction and rehabilitation including stripping topsoil where required, installing compacted gravel base, shaping for drainage, and optional fabric or culverts.",
  },
  {
    title: "Land Clearing (Selective)",
    description: "Targeted removal of trees and vegetation to support building sites, access routes, and utility corridors without unnecessary disturbance.",
  },
  {
    title: "Timber & Stump Work",
    description: "Per-tree and per-stump services for oversized timber, hazard trees, and stumps, performed standalone or in combination with larger site work.",
  },
];

const process = [
  {
    step: 1,
    title: "Evaluate site conditions and access",
    description: "Assess terrain, slope, vegetation density, and equipment accessibility to determine feasibility and appropriate methods.",
  },
  {
    step: 2,
    title: "Confirm scope, slope, and constraints",
    description: "Define project boundaries, identify safety limitations, and establish work requirements based on site conditions.",
  },
  {
    step: 3,
    title: "Select appropriate methods and equipment",
    description: "Choose the right equipment and techniques based on slope, terrain, and project goals to ensure safe and effective execution.",
  },
  {
    step: 4,
    title: "Execute work safely and efficiently",
    description: "Perform the work with emphasis on safety protocols, equipment protection, and minimizing unnecessary site disturbance.",
  },
  {
    step: 5,
    title: "Leave the site stable and functional",
    description: "Ensure proper drainage, erosion control, and site stability for long-term durability and maintenance.",
  },
];

const serviceArea = {
  region: "Bitterroot Valley and surrounding Western Montana communities",
  description: "Serving property owners across Western Montana with professional land management and site services.",
};

export {
  services,
  process,
  serviceArea,
};
